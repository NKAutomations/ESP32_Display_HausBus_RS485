...
// Speichert den Status für jede Ebene einzeln: [Ebene][ButtonIndex]
static bool is_remote_active[3][6] = {
    {false, false, false, false, false, false}, // Ebene 0
    {false, false, false, false, false, false}, // Ebene 1
    {false, false, false, false, false, false}  // Ebene 2
};
...
static void handle_led_telegram(const HausbusTelegram &telegram) {
    // Wir prüfen nur noch, ob es ein LED-Befehl ist (ID-Prüfung hier weglassen!)
    if (telegram.function == "LED") {
        uint16_t instance = telegram.instanceId;
        bool value = (telegram.value == "1");

        if (instance >= 1 && instance <= 6) {
            // 1. Bestimme, zu welcher Ebene diese DeviceID gehört
            int targetLevel = -1;
            ...
            // Wenn die ID gültig ist, speichern wir den Zustand IMMER im Speicher
            if (targetLevel != -1) {
                is_remote_active[targetLevel][instance - 1] = value;

                // 2. Nur wenn das Telegramm zur AKTUELL gewählten Ebene gehört, 
                // aktualisieren wir die Anzeige sofort auf dem Display.
                if (targetLevel == get_selected_level()) {
                    ...
                    if (targetBtn != nullptr) {
                        if (value) {
                            set_button_color_pressed(targetBtn, true);  // Orange
                        } else {
                            set_button_color_released(targetBtn);         // Default
                        }
                    }
                }
            }
...
static void handle_button_event(lv_event_t *e, lv_obj_t *btn, uint8_t buttonInstance) {
    ...
    if (code == LV_EVENT_PRESSED) {
        // Wenn der Button NICHT remote aktiv ist, färbe ihn kurz GRÜN
        if (!is_remote_active[get_selected_level()][buttonInstance - 1]) {
            set_button_color_pressed(btn, false); // false = Lokal (Grün)
        } else {
            // Wenn er remote aktiv ist, bleibt er beim Drücken Orange
            set_button_color_pressed(btn, true);  // true = Remote (Orange)
        }
        send_button_command(buttonInstance, 1);
    }
    else if (code == LV_EVENT_RELEASED || code == LV_EVENT_PRESS_LOST) {
        // Beim Loslassen immer auf Default zurücksetzen, 
        // ES SEI DENN er ist remote aktiv.
        if (!is_remote_active[get_selected_level()][buttonInstance - 1]) {
...
static void dropdown_event_cb(lv_event_t *e) {
    if (lv_event_get_code(e) == LV_EVENT_VALUE_CHANGED) {
        ...
        uint16_t level = get_selected_level();
        lv_obj_t *btns[6] = {
            objects.btn_1, objects.btn_2, objects.btn_3, 
            objects.btn_4, objects.btn_5, objects.btn_6
        };

        for (int i = 0; i < 6; i++) {
            if (btns[i] != nullptr) {
                // Prüfe den Speicher der NEUEN Ebene
                bool active = is_remote_active[level][i];
                if (active) {
                    set_button_color_pressed(btns[i], true);  // Orange für neue Ebene
                } else {
                    set_button_color_released(btns[i]);         // Default für neue Ebene
                }
            }
        }
